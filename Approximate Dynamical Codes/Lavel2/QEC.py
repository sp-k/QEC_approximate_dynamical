    ...
    def run(self, _ATOL = 1e-2):
        ...
        while(True):
            ...
            for chk in self.chk_op:
                obj = array(self.enc_op)[0][0]*tr(array(chk)@kron(self.N0[0][0][0], self.get_cd(self.num_qubit, 0, 0)))*kron(self.N0[0][0][0], self.R1[0][0])
                for a in range(2**self.num_qubit):
                    for b in range(2**self.num_qubit):
                                ...
                                for s in range(2**self.num_qubit):
                                    for t in range(2**self.num_qubit):
                                        for i in range(2**self.num_qubit):
                                            obj += array(self.enc_op)[t*2**self.num_qubit+b][s*2**self.num_qubit+a]*tr(array(chk)@kron(self.N0[a][b][i], self.get_cd(self.num_qubit, c, d)))*kron(self.N0[c][d][i], self.R1[s][t])
                            stop = time()
                            print(f'Time for each b: {time()-start}')
                obj -= array(self.enc_op)[0][0]*tr(array(chk)@kron(self.N0[0][0][0], self.get_cd(self.num_qubit, 0, 0)))*kron(self.N0[0][0][0], self.R1[0][0])
                Obj.append(obj)
...
#             print(f'Iteration {itr}: Running SDP for checking... {fidelity} {self.fidelity}', end = ', ')
            ...
            for dec in self.dec_op:
                obj = array(self.enc_op)[0][0]*tr(array(dec)@kron(self.N0[0][0][0], self.R1[0][0]))*kron(self.N0[0][0][0], self.get_cd(self.num_qubit, 0, 0))
                for a in range(2**self.num_qubit):
                    for b in range(2**self.num_qubit):
                        for c in range(2**self.num_qubit):
                            for d in range(2**self.num_qubit):
                                for s in range(2**self.num_qubit):
                                    for t in range(2**self.num_qubit):
                                        for i in range(2**self.num_qubit):
                                            obj += array(self.enc_op)[t*2**self.num_qubit+b][s*2**self.num_qubit+a]*tr(array(dec)@kron(self.N0[c][d][i], self.R1[s][t]))*kron(self.N0[a][b][i], self.get_cd(self.num_qubit, c, d))
                obj -= array(self.enc_op)[0][0]*tr(array(dec)@kron(self.N0[0][0][0], self.R1[0][0]))*kron(self.N0[0][0][0], self.get_cd(self.num_qubit, 0, 0))
                Obj.append(obj)
            objective = variable[0]@Obj[0]
            for var, obj in zip(variable[1:], Obj[1:]):
...
#             print(self.fidelity, fidelity, end = '\r')
            ...
            mat[0][0] = 1
            Obj = tr(self.chk_op[0]@kron(self.N0[0][0][0], self.get_cd(self.num_qubit, 0, 0)))*tr(self.dec_op[0]@kron(self.N0[0][0][0], self.R1[0][0]))*mat
            for a in range(2**self.num_qubit):
                for b in range(2**self.num_qubit):
                    for c in range(2**self.num_qubit):
                        for d in range(2**self.num_qubit):
                                ...
                                for t in range(2**self.num_qubit):
                                    for i in range(2**self.num_qubit):
                                        for m in range(len(self.chk_op)):
                                            ...
                                            Obj += tr(self.chk_op[m]@kron(self.N0[a][b][i], self.get_cd(self.num_qubit, c, d)))*tr(self.dec_op[m]@kron(self.N0[c][d][i], self.R1[s][t]))*mat
            mat = zeros((2**(self.num_qubit<<1), 2**(self.num_qubit<<1)))
            mat[0][0] = 1
            Obj -= tr(self.chk_op[0]@kron(self.N0[0][0][0], self.get_cd(self.num_qubit, 0, 0)))*tr(self.dec_op[0]@kron(self.N0[0][0][0], self.R1[0][0]))*mat
...
#                     print(f'Time taken by {a} for b = {b}, c = {c}, d = {d}: {time()-start}')
#         lock.acquire()
#         obj += objL
#         lock.release()
...
#             print(f'Time taken by {a} for b = {b}')
#         lock.acquire()
#         obj += objL
#         lock.release()
...
#         chk.append(Operator(qc).compose(kron(eye(2), M0)))
#         chk.append(Operator(qc).compose(kron(eye(2), M1)))
#         return [Operator(eye(2))]#chk#[Operator(eye(4))]
