...
from QEC_parallel import n_qubit_code
...
with open('choi', 'wb') as file:
    ...
    if 'enc' not in list(data[f'{num_qubit}-qubit']['2-tensor'][num_mem].keys()):
...
def exec_QEC(state, i):
    ...
    with open('choi', 'wb') as file:
        dump(data, file)
    fid_SDPn[i] += fid
    print(i, damp_params[i], fid_SDPn[i], enc, dec)
...
if '2-tensor' not in list(data[f'{num_qubit}-qubit'].keys()):
    data[f'{num_qubit}-qubit']['2-tensor'] = {}
data[f'{num_qubit}-qubit']['2-tensor'][num_mem] = {f'fid_SDP{num_qubit}': fid_SDPn, 'fid_sing': fid_sing}
