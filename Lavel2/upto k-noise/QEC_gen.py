                ...
                for d in range(2**self.num_qubit):
                    for s in [0, 2**(~-self.num_qubit)]:	# For other values, the corresponding R1 entry is 0
                        for t in [0, 2**(~-self.num_qubit)]:	# For other values, the corresponding R1 entry is 0
                            for i in range(2**self.num_error*comb(self.num_qubit, self.num_error)):
                            	for j in range(2**self.num_error):
                            		# The relation between i and j ensures that the noise is applied on the same qubit in the both part
                            		if (i%2**self.num_error)&j:
                            			continue
                            		j += i-i%2**self.num_error
                            		tra = 0
                            		...
                            		objL += array(self.enc_op)[t*2**self.num_qubit+b][s*2**self.num_qubit+a]*tra*kron(self.N1[c][d][j], self.R1[s][t])
                            ...
                            for i in range(2**self.num_error*comb(self.num_qubit, self.num_error)):
                            	for j in range(2**self.num_error):
                            		...
                            		if (i%2**self.num_error)&j:
                            			continue
                            		j += i-i%2**self.num_error
                            		objL += array(self.enc_op)[t*2**self.num_qubit+b][s*2**self.num_qubit+a]*trace(array(dec)@kron(self.N1[c][d][j], self.R1[s][t]))*kron(self.N0[a][b][i], self.get_cd(self.num_qubit, c, d))
                            ...
                            for i in range(2**self.num_error*comb(self.num_qubit, self.num_error)):
                            	for j in range(2**self.num_error):
                            		...
                            		for m in range(len(self.chk_op)):
		                                ...
		                                objL += tra*trace(self.dec_op[m]@kron(self.N1[c][d][j], self.R1[s][t]))*mat
    ...
    def run(self, _ATOL = 1e-2):
        ...
        while(True):
            ...
            for chk in self.chk_op:
                ...
                shm = shared_memory.SharedMemory(create = True, size = d_size, name = 'object')	# creates shared memory
